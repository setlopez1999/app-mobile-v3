import 'dart:async';
import 'package:flutter/cupertino.dart';
import 'package:fpdart/fpdart.dart';
import 'package:go_router/go_router.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';
import 'package:tvapp/core/application/states/auth/auth_state.dart';
import 'package:tvapp/core/application/use_cases/auth/clean_session_use_case.dart';
import 'package:tvapp/core/application/use_cases/auth/get_login_info_use_case.dart';
import 'package:tvapp/core/application/use_cases/auth/get_session_use_case.dart';
import 'package:tvapp/core/application/use_cases/auth/login_token_use_case.dart';
import 'package:tvapp/core/application/use_cases/auth/login_use_case.dart';
import 'package:tvapp/core/application/use_cases/auth/save_session_use_case.dart';
import 'package:tvapp/core/domain/entities/contact/contact_entity.dart';
import 'package:tvapp/core/domain/entities/sensitive_data/sensitive_data_entity.dart';
import 'package:tvapp/core/domain/entities/user/user_entity.dart';
import 'package:tvapp/core/infraestructure/repositories/auth_http_repository.dart';
import 'package:tvapp/core/shared/exceptions/app_exception.dart';
import 'package:tvapp/ui/screens/login/login.screen.dart';

part 'auth_provider.g.dart';

@Riverpod(keepAlive: true)
class Auth extends _$Auth {
  final repository = AuthHttpRepository();
  final failure = const SensitiveDataEntity(parental: '', token: '', email: '', userID: '', deviceId: '');
  Timer? _timer;

  @override
  AuthState build() {
    _startSessionMonitoring();

    ref.onDispose(() {
      _timer?.cancel();
    });

    return const AuthState.initial();
  }

  void _startSessionMonitoring() {
    _timer?.cancel();

    _timer = Timer.periodic(const Duration(minutes: 1), (_) async {
      await loadSession(silent: true);
    });
  }

  Future<Either<AppException, Contact>> getLoginInfo() async {
    final loginData = await GetLoginInfoUseCase(repository).execute();
    return loginData;
  }

  Future<void> login(String email, String password) async {
    state = const AuthState.loading();
    final useCase = LoginUseCase(repository);

    final result = await useCase.execute(email, password);

    result.fold(
      (error) => state = AuthState.error(error),
      (user) => state = AuthState.success(user),
    );
  }

  void saveSession(String email) {
    final user = state.maybeWhen(
      success: (user) => user,
      orElse: () => null,
    );

    if (user != null) {
      final data = SensitiveDataEntity(
        token: user.token,
        email: email,
        parental: user.parentlockcode,
        userID: user.us_id,
        deviceId: user.devid
      );
      SaveSessionUseCase(repository).execute(data);
    }
  }
  
  Future<void> loadSession({bool silent = false }) async {
    final useCase = GetSessionUseCase(repository);
    final session = await useCase.execute();

    if(session.isRight()) {
      final data = session.getOrElse((AppException e) => failure);

      if(data.token.isEmpty) {
        await CleanSessionUseCase(repository).execute();
        state = const AuthState.initial();
        return;
      }

      final loginData = await LoginTokenUseCase(repository).execute(data.token);
      loginData.fold(
        (_) async {
          state = const AuthState.initial();
          await CleanSessionUseCase(repository).execute();
        },
        (user) => silent ? null : state = AuthState.success(user),
      );
    }
  }

  Future<void> logout() async {
    await CleanSessionUseCase(repository).execute();
    resetAllProviders();
    state = const AuthState.initial();
  }

  Future<(User, SensitiveDataEntity)> getAllUserInfo() async {

    final user = state.maybeWhen(
      success: (user) => user,
      orElse: () => null,
    );

    final sessionUseCase = await GetSessionUseCase(repository).execute();
    final session = sessionUseCase.getOrElse((AppException e) => failure);

    return (user!, session);
  }

  void resetAllProviders() {
    ref.container.pump();
  }
}
